...
//目前先用survey1.js文件代替后端的文卷数据，并假定得到的问卷类型是1！！！！！
import surveyData from '../PiniaStores/survey1.js'
//目前先用survey1.js文件代替后端的文卷数据，并假定得到的问卷类型是1！！！！！
axios({
  url: `https://q.denglu1.cn/user/fillQuestionnaire/${1}`, //目前问卷id有1、2、3
  method: 'get',
  ...
  //网络请求成功，写入问卷类型和问卷数据到currentSurvey
  currentSurvey.type = 1
  currentSurvey.data = surveyData[0]
}).catch((error) => {
  console.log(error)
})
...
<template>
  <div class="wrapper">
    ...
    <!--动态组件-->
    <component :is="surveyTemplateList[currentSurvey.type - 1]"></component>
    <!--动态组件-->
