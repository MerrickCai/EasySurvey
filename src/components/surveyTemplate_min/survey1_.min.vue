...
for (let i in surveyObj.value.answerMap) {
  let obj = {}
  obj.ques = surveyObj.value.questionInfoMap[i].info;
  obj.questionId = i;
  obj.answerValue = surveyObj.value.answerMap[i][0].optionId
  obj.optionList = surveyObj.value.optionMap[i]
  obj.value = surveyObj.value.optionMap[i][surveyObj.value.optionMap[i].length - 1].id - obj.answerValue;
  obj.font = [];
...
const barArr = new Array(survey.quesList.length)
  .fill(0)
  .map((item, index) =>
    new Array(survey.quesList[index].optionList.length).fill(0)
);
