<template>
  ...
  <div class="wrapper" v-if="jump === 1">
    ...
    <el-button type="primary" class="btn" @click="toContent"
      >开始问卷</el-button
    >
  ...
  <div class="wrapper extrachange" v-if="jump === 2">
    ...
    <div class="progress" @click="scrollTo($event)">
      <div>
        ...
        <div
          class="progress-part"
          v-for="item of survey.questionList"
          :key="item.id"
          :style="{
    ...
    <div class="content" ref="content" @scroll="onScroll($event)">
      ...
      <div
        class="main"
        v-for="(item, i) of survey.questionList"
        :key="item.id"
      ...
      >
        ...
        <div
          class="questionList"
          v-for="(elem, index) of item.question"
          :key="index"
        >
          {{ elem.detail }}
          <div class="slide" :style="{ backgroundColor: `${item.bcg}` }">
            ...
            <div
              class="Gradient"
              :style="{
                width: `${elem.value * (400 / (barArr[i].length - 1))}px`,
              }"
            ></div>
            <!-- 第三层循环 b,j -->
            <div
              v-for="(b, j) of barArr[i]"
              :key="i"
              :class="j % 2 === 0 ? 'doublebar' : 'bar'"
            ...
            <img
              class="thumb"
              :style="{
                left: `${elem.value * (400 / (barArr[i].length - 1))}px`,
              }"
            ...
            <span
              class="edit"
              v-show="!elem.isEdit"
              @click="editHandle(elem, index)"
              >{{ elem.value }}<img src="/icon-edit.png"
            /></span>
            <input
              class="editinput"
              type="text"
              v-show="elem.isEdit"
              @blur="editHandle2(elem, item, $event)"
              @keydown.enter="editHandle2(elem, item, $event)"
              :value="elem.value"
              ref="myRef"
            />
          </div>
  ...
  <div class="finish-wrapper" v-if="jump === 3">
    <div class="innerbox">
      ...
      <el-button type="primary" class="finish-submit">{{
        survey.end.button
      }}</el-button>
    </div>
  </div>
...
const barArr = new Array(survey.value.questionList.length)
  .fill(0)
  .map((item, index) => new Array(survey.value.questionList[index].score + 1));
...
function onScroll(e) {
  ...
  thumb.value.setAttribute(
    "style",
    `top: ${300 * (content.value.scrollTop / scrollDistence.value) - 8}px`
  );
  text.value.innerHTML = `${Math.ceil(
    (content.value.scrollTop / scrollDistence.value) * 100
  )} %`;
